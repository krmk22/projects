...
unsigned char GSM_Identifier(unsigned char State)
{
  Serial_Write("AT+CLIP=");
  Serial_Send(State ?'1' :'0');
  Serial_Flush();
  return STalkBack(True, "OK", 2, 2500);
}

unsigned char GSM_Initialize(unsigned long ReqBaud)
{
...
unsigned char GSM_EnableGPRS(unsigned char State)
{
  ...
  Serial_Send(State ?'1' :'0');
  return STalkBack(True, "OK", 2, 5000);  
}
...
unsigned char GSM_StartGPRS(const char *ServerName, const char *Port)
{
  Serial_Write("AT+CIPSTART=");
  Serial_Write("\"TCP\",\"");
  Serial_Write(ServerName);
  Serial_Write("\",");
  Serial_Write(Port);
  Serial_Flush();
