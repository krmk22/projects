...
void main()
{
	...
	NRF_Initialize(45);
	NRF_TXRAddress((char*)RAddress1);
	NRF_RXRAddress(1, (char*)TAddress);
	memcpy(SerialArray, "HELLO", 5);
	NRF_WriteAckPayload(1, SerialArray, 5);
	...
	while(1)
	{
		...
		if(NRF_Receive(SerialArray, &SerialCount))
		{
			LCD_Write(0x8F, 'R');
	    	...
	    	SerialCount = 5;
			NRF_WriteAckPayload(1, SerialArray, SerialCount);
		}
