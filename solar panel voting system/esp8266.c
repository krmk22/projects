...
unsigned char ESP_Echo(unsigned char State)
{
	Serial_Write("ATE");
...
unsigned char ESP_Initialize(unsigned long ReqBaud)
{
	...
	Serial_Enable(True);
	ESP_Reset(); DelayMS(2500);
	
	if(strstr((char*)SerialArray, "WIFI CONNECTED") != '\0')
	while(strstr((char*)SerialArray, "WIFI GOT IP"));
...
unsigned char ESP_WiFi()
{
	...
	Serial_Write("\"");
	Serial_Flush();
	return STalkBack(True, "WIFI GOT IP", 11, 25000);
}
