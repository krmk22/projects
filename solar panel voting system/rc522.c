...
volatile unsigned long LastTimer, RFInterval;
static int SlaveSelect;

void RegisterWrite(unsigned char Address, unsigned char Value)
{
	PORT_LOW(SlaveSelect);
	SPI_Write((Address << 1) & 0x7E);
	SPI_Write(Value);
	PORT_HIGH(SlaveSelect);
}
...
unsigned char RFID_Initialize(char SS, char SCK, char MOSI, char MISO, unsigned long Interval)
{
	RFInterval = Interval;
	SlaveSelect = SS;
	TRIS_OUTPUT(SlaveSelect);
	PORT_HIGH(SlaveSelect); DelayMS(5);
	
	SPI_Initialize(SCK, MOSI, MISO);	
	
	RegisterWrite(0x01, 0x0F); DelayMS(50);
  	RegisterWrite(0x2A, 0x8D); DelayMS(20);
  	RegisterWrite(0x2B, 0x3E); DelayMS(20);
  	RegisterWrite(0x2C, 0x00); DelayMS(20);
  	RegisterWrite(0x2D, 0x30); DelayMS(20);
  	RegisterWrite(0x15, 0x40); DelayMS(20);
  	RegisterWrite(0x11, 0x3D); DelayMS(20);
  	RegisterWrite(0x26, 0x70); DelayMS(20);
  	RegisterMaskS(0x14, 0x03); DelayMS(20);
	
	if(!TMR3ON) Timer_Initialize();
	LastTimer = RFInterval;
...
unsigned char ReadRFID()
{
	...
	if(LastTimer < Millis())
	{
		...
		if(RequestTag(0x26, TagArray) == RFID_OK)
		{
			...
			for(unsigned char j = 0, k = 0; j < 5, k < 10; j += 1, k += 2)
			{
			...
			ReturnVariable = RFID_NOTAG;
		}
		LastTimer = Millis() + RFInterval;
	}	
	return ReturnVariable;
}		 	
